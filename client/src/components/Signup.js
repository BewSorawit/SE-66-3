            ...
            <div className="bg-white p-5 rounded w-50 mt-3 mt-3">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        ...
                        <input id="userID" type="text" placeholder='Enter an id user ( only for admin do it )' name='userID' className='form-control rounded-0' onChange={handleInput} />
                        {errors.userID && <span className='text-danger'> {errors.userID} </span>}
                    </div>

                    <div className="mb-3">
                        <label htmlFor='firstName'><strong>First Name : </strong></label>
                        <input id="firstName" type="text" placeholder='Enter First Name' name='firstName' className='form-control rounded-0' onChange={handleInput} />
                        {errors.firstName && <span className='text-danger'> {errors.firstName} </span>}
                    </div>

                    <div className="mb-3">
                        <label htmlFor='surName'><strong>SurName : </strong></label>
                        <input id="surName" type="text" placeholder='Enter Surname' name='surName' className='form-control rounded-0' onChange={handleInput} />
                        {errors.surName && <span className='text-danger'> {errors.surName} </span>}
                    </div>

                    <div className="mb-3">
                        <label htmlFor='email'><strong>Email : </strong></label>
                        <input id="email" type="email" placeholder='Enter Email' name='email' className='form-control rounded-0' onChange={handleInput} />
                        {errors.email && <span className='text-danger'> {errors.email} </span>}
                    </div>

                    <div className="mb-3">
                        <label htmlFor='date'><strong>Date birth day : (02/15/2014) </strong></label>
                        <input id="date" type="date" placeholder='select date' name='dateBirth' className='form-control rounded-0' onChange={handleInput} />
                        {errors.dateBirth && <span className='text-danger'> {errors.dateBirth} </span>}
                    </div>

                    <div className="mb-3">
                        <label htmlFor='passwordUser'><strong>Password :  </strong></label>
                        <input id="passwordUser" type="text" placeholder='Enter the password' name='passwordUser' className='form-control rounded-0' onChange={handleInput} />
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            <div className="row mb-3">
                                <div className="form-group col-md-4">
                                    <select id="roleID" name='roleID' className='form-select' onChange={(e) => setValues({ ...values, roleID: e.target.value })}>
